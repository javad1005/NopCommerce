...
namespace Nop.Plugin.Api.Attributes
{
    public class ImageValidationAttribute : BaseValidationAttribute
    {
        ...
        public override async Task ValidateAsync(object instance)
        {
            ...
            if (imageSrcSet || imageAttachmentSet)
            {
                ...
                if (_errors.Count == 0)
                {
                    ...
                    //NopComment add image name parameters 
        ...
        private async Task ValidatePictureByteArrayAsync(byte[] imageBytes, string mimeType, string imageName)
        {
            if (imageBytes != null)
            {
                try
                {
                    //NopComment add image name parameters 
                    imageBytes = await _pictureService.ValidatePictureAsync(imageBytes, mimeType, imageName);
                }
                catch (Exception ex)
                {
